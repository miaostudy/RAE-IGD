...
MEANS = {'cifar': [0.4914, 0.4822, 0.4465], 'imagenet': [0.5, 0.5, 0.5]}  # [0.485, 0.456, 0.406]
STDS = {'cifar': [0.2023, 0.1994, 0.2010], 'imagenet': [0.5, 0.5, 0.5]} # [0.229, 0.224, 0.225]
...
class ImageFolder_mp2(datasets.DatasetFolder):
    ...
    def __init__(self,
        ...
        super(ImageFolder_mp, self).__init__(root,
                                          loader,
                                          self.extensions,
                                          transform=transform,
        ...
        if nclass < 1000:
            self.classes, self.class_to_idx = self.find_subclasses(nclass=nclass,
                                                                   phase=phase,sel_class=sel_class)
        else:
...
class CIFAR10_mp(datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None,
        ...
        if self.sel_class != 'none':
            # sel_class 传进来的可能是数字字符串 '0' 或者类别名 'airplane'

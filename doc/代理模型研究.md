...
训练
```shell
CUDA_VISIBLE_DEVICES=0,1 torchrun --nnode=1 --nproc_per_node 2 --master_port=25678 MinimaxDiffusion/train_dit.py --model DiT-XL/2      --data-path /data/wlf/datasets/imagenet/train/ --ckpt pretrained_models/DiT-XL-2-256x256.pt      --global-batch-size 8 --tag minimax --ckpt-every 12000 --log-every 1500 --epochs 10      --condense --finetune-ipc -1 --results-dir ../logs/run-imagenette-0 --dataset imagenet --spec nette
```
采样
```shell
python MinimaxDiffusion/sample.py --model DiT-XL/2 --image-size 256 --ckpt ../logs/run-imagenette-0/0012000.pt \
...
### dinov2-base
```shell
python IGD/train_ckpts.py -d imagenet -n dinov2-base --nclass 10 --norm_type instance --tag test --slct_type random --mixup vanilla --repeat 1 --ckpt-dir ./ckpts/imagenette/dinov2-base/ --epochs 50 --ipc -1 --imagenet_dir /data/wlf/datasets/imagenette --spec nette
```
...
### resnet18-ipc10-dit
```shell
python IGD/train.py -d imagenet --imagenet_dir ./results/dit-distillation/nette-10-minimax-igd-ckpts-resnet10-k15-gamma10-r1-gi200-low30-high45 /data/wlf/datasets/imagenette/ -n resnet --depth 18 --nclass 10 --norm_type instance --ipc 10 --tag test --slct_type random --spec nette --batch_size 32 --verbose
```
...
训练
```shell
CUDA_VISIBLE_DEVICES=0,1 torchrun --nnode=1 --nproc_per_node 2 --master_port=25378 MinimaxDiffusion/train_dit.py --model DiT-XL/2      --data-path /data/wlf/datasets/imagenet/train/ --ckpt pretrained_models/DiT-XL-2-256x256.pt      --global-batch-size 8 --tag minimax --ckpt-every 12000 --log-every 1500 --epochs 10 --condense --finetune-ipc -1 --results-dir ../logs/run-imageWoof-0 --dataset imagenet --spec woof
```
## ckpt
...
## 采样
```shell
python IGD/sample_mp.py --model DiT-XL/2 --image-size 256 --ckpt pretrained_models/woof.pt \
    --save-dir "./results/dit-distillation/woof-10-dit-igd-ckpts-resnet18-k5-gamma120-r1-gi200-low30-high45" --data-path /data/wlf/datasets/imagewoof/train \
    --spec woof --gm-scale 5 --grad-ipc 200 --net-type resnet --depth 18 \
    --low 30 --high 45 --ckpt_path ckpts --repeat 1 --num-samples 10 --dev-scale 120 \
    --nclass 10 --phase 0 --target_nclass 10
```

## resnet10-ipc10-minimax
```shell
python IGD/train.py -d imagenet --imagenet_dir ./results/dit-distillation/woof-10-dit-igd-ckpts-resnet18-k5-gamma120-r1-gi200-low30-high45 /data/wlf/datasets/imagewoof/ -n resnet --depth 18 --nclass 10 --norm_type instance --ipc 10 --tag test --slct_type random --spec woof --batch_size 32
```
# RAE-IGD
